...
Blood blood(0, 0);
IMAGE background, ship, ship1, enemy, enemy1, smallenemy, smallenemy1, life, life1;
...
int HandleGameOver()
{
	if (GameOver())
	{
		...
		loadimage(&background, _T("res\\gameover.jpg"), GAMEWIDTH, GAMEHIGHT, true);
		// 清空数组
...
void MakeEnemyBullet1()
{
	if (myenemy.m_Fire % 20 == 0)
	{
		// 创造敌机子弹
		CBullet * pNewEnemyBullet = new CBullet((myenemy.m_nRow + enemy.getwidth() / 2), myenemy.m_nCol + enemy.getheight());
		VectorEnenyBullets.push_back(pNewEnemyBullet);
